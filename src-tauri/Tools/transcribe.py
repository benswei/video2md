...
# 自动安装完成后再导入第三方依赖（避免首行 import 在依赖缺失时直接崩溃）
import requests
...
def optimize_to_markdown(text, provider="gemini", api_url=None, api_key=None, model_name=None, prompt_template=None):
    ...
    if provider == "gemini":
        ...
        if not api_key:
            raise RuntimeError("未配置 GEMINI_API_KEY，无法执行 Markdown 整理。")
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"安装 google-genai 失败: {e}")
        ...
        try:
            ...
            response = call_with_retry(
                client.models.generate_content,
                model=(model_name.strip() if model_name else "gemini-2.5-flash"),
                contents=[prompt]
            )
            optimized_text = response.text.strip()
        except Exception as e:
            raise RuntimeError(f"Gemini Markdown 整理失败: {e}")
    else:
        # 自定义 OpenAI 兼容大模型
        if not api_url or not api_key:
            raise RuntimeError("自定义 Markdown 整理未配置 Endpoint 或 API Key。")
        ...
        try:
            ...
            detail = response.text.strip().replace("\n", " ")[:800] if 'response' in locals() else ""
            raise RuntimeError(f"自定义 Markdown 整理失败：HTTP {e.response.status_code if e.response else '错误'}。{detail}")
        except Exception as e:
            raise RuntimeError(f"自定义 Markdown 整理失败: {e}")
...
def try_fetch_subtitles(url, tmpdir, cookies_from_browser=None, proxy=None):
    ...
    成功返回字幕文本，失败返回 None
    """
    if ("bilibili.com" in url or "b23.tv" in url) and (not cookies_from_browser or cookies_from_browser.lower() == "none") and not os.environ.get("YOUTUBE_COOKIES_FILE"):
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    yt_cookies_path = os.environ.get("YOUTUBE_COOKIES_FILE") or os.path.join(os.path.dirname(script_dir), "youtube_cookies.txt")
...
def save_outputs(text_or_result, output_base_path, formats, engine, source_input=None, video_title=None,
    ...
    if llm_provider and llm_provider != "none" and ("md" in formats_list or "markdown" in formats_list or "html" in formats_list):
    ...
    for fmt in formats_list:
        ...
        with open(out_path, "w", encoding="utf-8") as f:
            if fmt == "md" or fmt == "markdown":
                ...
                # 自适应可视化 HTML 模板（响应式 + 目录 + 搜索 + 暗色 + 打印）
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .search { flex: 1; max-width: 320px; padding: 7px 12px; border: 1px solid var(--border-color); border-radius: 8px; background: var(--bg-color); color: var(--text-color); font-size: 13px; outline: none; }
        .search:focus { border-color: var(--primary-color); }
        .tbtn { width: 34px; height: 34px; border: 1px solid var(--border-color); border-radius: 8px; background: var(--bg-color); color: var(--text-color); cursor: pointer; font-size: 15px; }
        .tbtn:hover { border-color: var(--primary-color); }
        .layout { display: flex; gap: 28px; max-width: 1180px; margin: 0 auto; padding: 24px 20px 64px; }
        .sidebar { width: 220px; flex-shrink: 0; position: sticky; top: 64px; align-self: flex-start; max-height: calc(100vh - 84px); overflow-y: auto; }
        .toc-title { font-size: 11px; font-weight: 700; color: var(--muted); text-transform: uppercase; letter-spacing: .06em; margin-bottom: 8px; }
        .toc a { display: block; font-size: 13px; color: var(--muted); text-decoration: none; padding: 4px 8px; border-radius: 6px; line-height: 1.4; border-left: 2px solid transparent; }
        .toc a:hover { color: var(--primary-color); background: var(--code-bg); }
        .toc a.active { color: var(--primary-color); border-left-color: var(--primary-color); font-weight: 600; }
        ...
        h1, h2, h3, h4 { color: var(--heading-color); margin-top: 1.8em; margin-bottom: 0.8em; font-weight: 700; scroll-margin-top: 72px; }
        h1 { font-size: 2rem; border-bottom: 2px solid var(--border-color); padding-bottom: 0.5rem; margin-top: 0; }
        h2 { font-size: 1.5rem; border-bottom: 1px solid var(--border-color); padding-bottom: 0.4rem; }
        h3 { font-size: 1.2rem; }
        p { margin-top: 0; margin-bottom: 1.1em; }
        a { color: var(--primary-color); text-decoration: none; }
        a:hover { text-decoration: underline; }
        strong { color: var(--heading-color); }
        ul, ol { margin-top: 0; margin-bottom: 1.1em; padding-left: 1.5rem; }
        li { margin-bottom: 0.35em; }
        blockquote { border-left: 4px solid var(--primary-color); background: color-mix(in srgb, var(--primary-color) 6%, transparent); padding: 0.8rem 1.2rem; margin: 1.4em 0; border-radius: 0 8px 8px 0; }
        ...
        pre { background: var(--code-bg); padding: 1rem 1.1rem; border-radius: 8px; overflow-x: auto; margin: 1.4em 0; border: 1px solid var(--border-color); }
        ...
        hr { border: none; border-top: 1px solid var(--border-color); margin: 1.6em 0; }
        .meta-info { font-size: 0.85rem; color: var(--muted); margin-bottom: 1.6rem; display: flex; gap: 1.4rem; flex-wrap: wrap; border-bottom: 1px solid var(--border-color); padding-bottom: 1rem; }
        #backTop { position: fixed; right: 22px; bottom: 22px; display: none; z-index: 60; }
        ...
        @media print {
            .topbar, #backTop, .sidebar { display: none !important; }
            body { background: #fff; }
...
<body>
    <div class="topbar">
        <input id="searchBox" class="search" placeholder="页内搜索…" />
        <button id="darkToggle" class="tbtn" title="暗色切换">🌙</button>
        <button class="tbtn" title="打印" onclick="window.print()">🖨</button>
        <button id="backTop" class="tbtn" title="返回顶部">↑</button>
    </div>
    <div class="layout">
        <aside class="sidebar">
    ...
    (function(){
        ...
        heads.forEach(function(h){
            if(!h.id){ h.id = h.textContent.trim().slice(0,40).replace(/[^\\w\\u4e00-\\u9fa5]+/g,'-'); }
        ...
        dt && dt.addEventListener('click', function(){ document.body.classList.toggle('dark'); });
        var sb = document.getElementById('searchBox');
        sb && sb.addEventListener('input', function(){
            var q = sb.value.trim().toLowerCase();
            var blocks = content ? content.children : [];
            for(var i=0;i<blocks.length;i++){
                var t = blocks[i].textContent.toLowerCase();
                blocks[i].style.display = (!q || t.indexOf(q) >= 0) ? '' : 'none';
            }
        });
        var bt = document.getElementById('backTop');
        bt && bt.addEventListener('click', function(){ window.scrollTo({top:0,behavior:'smooth'}); });
        window.addEventListener('scroll', function(){ bt.style.display = window.scrollY > 400 ? 'block' : 'none'; });
    })();
    </script>
</body>
</html>
"""
                html_template = html_template.replace("__TITLE__", title_display).replace("__META__", meta_header).replace("__BODY__", html_body)
...
def main():
    ...
    # 第三方大语言模型 (LLM) 整理配置
    parser.add_argument("--llm-provider", default="none", choices=["none", "gemini", "custom", "cli"], help="AI 大模型提供商")
